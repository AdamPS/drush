...
class StatusInfoDrush implements StatusInfoInterface {
  ...
  private function getAvailableReleases($projects) {
    drush_log(dt('Checking available update data ...'), LogLevel::INFO);
    ...
    foreach ($projects as $project_name => $project) {
      ...
      drush_log(dt('Checking available update data for !project.', array('!project' => $project['label'])), LogLevel::INFO);
      $request = pm_parse_request($project_name, NULL, $projects);
